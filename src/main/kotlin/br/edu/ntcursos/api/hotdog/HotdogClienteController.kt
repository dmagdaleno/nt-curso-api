...
@RestController
@RequestMapping("/hotdog/clientes")
class HotdogClienteController(
    val repositorio: HotdogClienteRepository
) {
    ...
    @GetMapping
    fun listaTodos(): ResponseEntity<List<HotdogCliente>> {
    ...
    @GetMapping("/{id}")
    fun busca(@PathVariable id: Long): ResponseEntity<HotdogCliente> {
    ...
    @PostMapping
    fun cria(@RequestBody cliente: HotdogCliente): ResponseEntity<HotdogCliente> {
    ...
    @PutMapping
    fun altera(@RequestBody cliente: HotdogCliente): ResponseEntity<HotdogCliente> {
        val saved = repositorio.save(cliente)
        val uri = montaURI(saved.id)
        return ResponseEntity.created(uri).build()
    }
