...
@RestController
@RequestMapping("/terapia/clientes")
class TerapiaClienteController(
    val repositorio: TerapiaClienteRepository
) {
    ...
    @GetMapping
    fun listaTodos(): ResponseEntity<List<TerapiaCliente>> {
    ...
    @GetMapping("/{id}")
    fun busca(@PathVariable id: Long): ResponseEntity<TerapiaCliente> {
    ...
    @PostMapping
    fun cria(@RequestBody cliente: TerapiaCliente): ResponseEntity<TerapiaCliente> {
    ...
    @PutMapping
    fun altera(@RequestBody cliente: TerapiaCliente): ResponseEntity<TerapiaCliente> {
        val saved = repositorio.save(cliente)
        val uri = montaURI(saved.id)
        return ResponseEntity.created(uri).build()
    }
